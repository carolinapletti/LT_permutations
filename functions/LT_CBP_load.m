...
% Carolina Pletti, 2025 (carolina.pletti@gmail.com)
    ...
    % ---------------------------------------------------------------------
    % For laughter data: average the two trials. Otherwise, extract data
    % ---------------------------------------------------------------------   
    ...
    if length(data) == 2
        ...
        % Define a function to truncate and average matrices
        truncate_and_average = @(v1, v2) ...
        mean(cat(3, ...
                 v1(1:min(size(v1, 1), size(v2, 1)), 1:min(size(v1, 2), size(v2, 2))), ...
                 v2(1:min(size(v1, 1), size(v2, 1)), 1:min(size(v1, 2), size(v2, 2)))), ...
                 3, 'omitnan');

        % Apply the function using cellfun
        averaged_videos = cellfun(@(v1, v2) truncate_and_average(v1, v2), video_1, video_2, 'UniformOutput', false);
        data = averaged_videos;
