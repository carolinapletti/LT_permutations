function [diffmap_cell, permmaps_cell] = LT_CBP_permutations(data_cell, data_RPA_cell, n_permutes, fqs, tps)
% LT_CBP_permutations
...
% and analysis: Zero to hero"
    ...
    for i = 1:length(data_cell)   
        ...
        channelMatrix_RPA = cat(3, data_RPA_cell{1,i}{:}); % RPA (random pairs) data
        channelMatrix_all = cat(3, channelMatrix, channelMatrix_RPA); % Combined data
        ...
        title(sprintf('Real Data (Averaged), channel %d', i))
        xlabel('Time points')
        ylabel('Periods (sec)')
        
        % Plot RPA data (averaged across random pairs)
        subplot(222)
        imagesc(tps,fqs,squeeze(mean( channelMatrix_RPA(:,:,:),3, "omitnan" )))
        title(sprintf('Random Pair Data (Averaged), channel %d', i))
        xlabel('Time points')
        ylabel('Periods (sec)')
        ...
        title(sprintf('Difference Map (Real - RPA), channel %d', i))
        xlabel('Time points')
        ylabel('Periods (sec)')
